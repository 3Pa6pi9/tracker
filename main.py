...
def save_items_bulk(items):
    ...
    for item in items:
        try:
            ...
            if c.rowcount > 0: 
                added += 1
                if item.get('is_priority_alert') and item['link'] not in ALERTED_PRIORITY_LINKS:
                    priority_found = True
                    priority_title = item['title']
                    priority_link = item['link']
                    ALERTED_PRIORITY_LINKS.add(item['link'])
        except Exception:
            pass
    conn.close()
...
@app.get("/api/ping")
def ping(): return {"status": "operational", "engine": "Telemetry Core 43.0"}

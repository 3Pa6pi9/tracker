...
app = FastAPI(title="Global Geopolitical Intelligence Command Center", version="1.0 - WebSocket Engine")
...
# If your DB wasn't connecting, Render needs this exact variable in its dashboard settings!
...
async def continuous_scraper_loop():
    while True:
        try:
            await manager.broadcast({"type": "log", "msg": "EXECUTING MAX-YIELD ASYNC SWEEP ACROSS GLOBAL FEEDS...", "alert": False})
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        # Send initial data instantly on connection
        try:
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute("SELECT * FROM news ORDER BY published_date DESC LIMIT 50")
